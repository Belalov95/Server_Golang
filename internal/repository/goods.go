...
func NewGoodsRepo(pool *pgxpool.Pool) *GoodsRepo {
	return &GoodsRepo{pool: pool}
}
func (g *GoodsRepo) ListStore(ctx context.Context) ([]models.Footballstore, error) {
	// выполнение SQL запроса через соединение с бд
	rows, err := g.pool.Query(ctx, `SELECT id, category, name, price FROM footballstore ORDER BY id`)
	if err != nil {
...
func (g *GoodsRepo) UpdateStore(ctx context.Context, updatedGoods *models.Footballstore) error {
	...
	result, err := g.pool.Exec(ctx, query, updatedGoods.ID, updatedGoods.Category, updatedGoods.Name, updatedGoods.Price)
	if err != nil {
...
func (g *GoodsRepo) InsertStore(ctx context.Context, newGood *models.Footballstore) error {
	...
	_, err := g.pool.Exec(ctx, query, newGood.Category, newGood.Name, newGood.Price)
	if err != nil {
